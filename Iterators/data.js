...
function listSongDetails(array){
    array.forEach(function(val){
        var details = songs.find(function(val2){
            return val.toLowerCase()===val2.name.toLowerCase();
        });
        console.log(details.name + ", by " + details.artist + "(" + details.duration + ")")
    });
}
...
//             return val.toLowerCase()===val2.name.toLowerCase();
//         });
//         if(details.month >5 && details.month <9){
//             count++
//         }
//     })
//     return count;
...
function getDurations(array){
    return array.map(function(val){
        return songs.find(function(val2){
            return val2.name.toLowerCase() === val.toLowerCase();
        }).duration;
    });
}

function getDurationInSeconds(array){
    return getDurations(array).map(function(val){
        var minsSecs = val.split(":");
        return (parseInt(minsSecs[0])*60 + parseInt(minsSecs[1])).toString();
    })
}

function getMainArtists(array){
    return array.map(function(val){
        var Obj = songs.find(function(val2){
            return val2.name.toLowerCase() === val.toLowerCase();
        });
        artistArray = Obj.artist.split(" featuring");
...
function getShortSongs(array){
    return array.filter(function(val){
        ...
        var length = obj.duration.split(":");
        return parseInt(length[0]) < 3;
    })
}
...
function getSongsByArtist(array,artist){
    return array.filter(function(val){
        ...
        return obj.artist.toLowerCase().includes(artist.toLowerCase());
    })
}
...
function getTotalDurationInSeconds(array){
    var secondsArray = getDurationInSeconds(array);
    return secondsArray.reduce(function(acc,val){
        return acc = parseInt(val) + parseInt(acc);
    })
}
...
function averageWeeksAtNumberOne(array){
    ...
    return array.reduce(function(acc,val){
        ...
        count++;
        return acc = obj.weeksAtNumberOne + acc;
    },0)/count;
}

console.log(averageWeeksAtNumberOne(names));